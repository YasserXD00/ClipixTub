...
export const DonationModal: React.FC<DonationModalProps> = ({ isOpen, onClose }) => {
    ...
    const detectCardBrand = (digits: string): string | null => {
        if (!digits) return null;

        // Visa: starts with 4, 13–19 digits (we'll treat 16+ as "complete")
        if (digits[0] === '4' && digits.length >= 13) return 'visa';
        ...
        const firstTwoStr = digits.slice(0, 2);
        if (digits.length === 15 && (firstTwoStr === '34' || firstTwoStr === '37')) {
            return 'amex';
        }

        // Discover (very simplified): 6011, 65, 644–649, 16 digits
        ...
        // Fallback: show a wallet / generic card provider only when it "looks" complete
        if (digits.length >= 14) {
            const altBrands = ['google_pay', 'apple_pay', 'amazon_pay'];
            return altBrands[Math.floor(Math.random() * altBrands.length)];
        }
    ...
    const handleCardInput = (e: React.ChangeEvent<HTMLInputElement>) => {
        const rawDigits = e.target.value.replace(/\D/g, '').slice(0, 19);

        // Format as XXXX XXXX XXXX XXXX XXXX
        const grouped = rawDigits.match(/.{1,4}/g)?.join(' ') ?? '';
    ...
    const getCardLogo = () => {
                ...
                case 'google_pay':
                    return (
                        <svg viewBox="0 0 40 20" className="h-6 w-auto">
                            <path fill="#4285F4" d="M6.3 8.6V4.4h1.7v4.2c0 .4-.1.7-.3.9-.2.2-.6.4-1 .4-.3 0-.6-.1-.8-.2-.2-.3-.5-.3-.9zM3.8 8.6V4.4h1.7v4.2c0 .5-.1.8-.4 1-.2.3-.6.4-1 .4-.4 0-.7-.1-1-.4-.2-.2-.3-.6-.3-1V4.4h1.7v3.5c0 .3.1.5.2.6.1.1.3.2.5.2.2 0 .4-.1.5-.2.1-.2.2-.4.2-.6z" />
        ...
        return <CreditCard className="w-5 h-5 text-slate-400 group-focus-within:text-red-500 transition-colors" />;
    };
            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-xl md:max-w-2xl bg-white dark:bg-slate-950 rounded-3xl shadow-2xl overflow-hidden font-sans border border-slate-800 flex flex-col max-h-[90vh]"
                    >
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.9 }}
                            onClick={onClose}
                            className="absolute top-3 right-3 md:top-4 md:right-4 p-2 bg-black/30 hover:bg-black/40 text-white rounded-full transition-colors z-20 backdrop-blur-md"
                        >
                            <X className="w-5 h-5" />
                        </motion.button>

                        {/* Header Background */}
                        <div className="h-32 md:h-40 bg-gradient-to-r from-red-700 via-red-600 to-black relative flex items-center justify-center shrink-0">
                            ...
                            <div className="absolute -bottom-12 flex flex-col items-center z-10">
                                <div className="w-24 h-24 bg-black rounded-full p-1.5 shadow-xl">
                                    <div className="w-full h-full bg-gradient-to-br from-red-600 to-slate-900 rounded-full flex items-center justify-center overflow-hidden border-4 border-slate-900">
                        ...
                        <div className="pt-16 pb-6 px-4 md:px-8 text-center text-slate-200 overflow-y-auto flex-1">
                            <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Enjoying ClipixTub?</h2>
                            <p className="text-slate-500 dark:text-slate-400 font-medium mb-4 md:mb-6 max-w-xl mx-auto">
                                ClipixTub is a nights-and-weekends project built by a single developer. There are no trackers,
                                no ads, and no corporate sponsor – just me trying to keep the servers, APIs and coffee running.
                                If this tool saves you time or helps with your work, a small tip genuinely makes a difference.
                            </p>
                            ...
                            {step === 'processing' ? (
                                ...
                                >
                                    ...
                                    <p className="text-slate-500 text-lg mb-8 max-w-sm mx-auto">
                                        Your support helps keep ClipixTub fast, up to date, and free for everyone. Thank you for being part of it.
                                    </p>
                                    <motion.button
                                        whileHover={{ scale: 1.05 }}
                                        whileTap={{ scale: 0.95 }}
                                        onClick={onClose}
                                        className="px-10 py-4 bg-gradient-to-r from-red-600 to-zinc-900 hover:from-red-500 hover:to-black text-white font-bold rounded-full transition-all shadow-lg hover:shadow-red-600/30"
                                    >
                                        Return to App
                                    </motion.button>
                                </motion.div>
                                ...
                                >
                                    <div className="bg-slate-50 dark:bg-slate-900/50 rounded-3xl p-5 md:p-7 border border-slate-200 dark:border-slate-800 mb-6 text-left shadow-inner">
                                        ...
                                        <div className="flex items-center gap-4 mb-6 md:mb-8">
                                            <div className="relative flex-grow">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-slate-950 border-2 border-slate-200 dark:border-slate-800 rounded-2xl text-lg font-bold outline-none focus:border-red-500 transition-colors"
                                                />
                                            </div>
                                            <div className="text-2xl font-black text-slate-300">=</div>
                                            <div className="min-w-[80px] text-center">
                                                <div className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-zinc-900">
                                                    ${amount}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-5">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                <div className="space-y-1">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full p-3.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm outline-none focus:border-red-500 transition-colors"
                                                    />
                                                </div>
                                                <div className="space-y-1">
                                            ...
                                            <div className="pt-2">
                                                <div className="p-5 bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group focus-within:ring-2 focus-within:ring-red-500/20 transition-all">
                                                    <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-red-600 via-red-500 to-black"></div>
                                    ...
                                    <div className="pb-4 sticky bottom-0 bg-white dark:bg-slate-950 pt-3 border-t border-slate-100 dark:border-slate-900">
                                        <motion.button
                                            whileHover={{ scale: 1.02 }}
                                            whileTap={{ scale: 0.98 }}
                                            onClick={handleDonate}
                                            className="w-full py-5 bg-gradient-to-r from-red-700 via-red-600 to-black hover:from-red-600 hover:via-red-500 hover:to-black text-white rounded-2xl font-black text-xl shadow-xl shadow-red-700/30 active:scale-[0.99] transition-all flex items-center justify-center gap-2"
                                        >
