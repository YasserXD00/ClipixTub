  ...
  return (
    <div className="mt-8 animate-fade-in">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="show"
        className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl border border-slate-200/50 dark:border-slate-800/50 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
      >
        {metadata.items.map((item) => (
          <motion.div
            variants={itemVariants}
            whileHover={{ backgroundColor: "rgba(230,0,0,0.02)" }}
            key={item.videoId}
            className={`p-4 flex items-center gap-4 border-b border-slate-100 dark:border-slate-800 last:border-0 transition-colors ${selected.has(item.videoId) ? 'bg-brand-50/50 dark:bg-brand-900/10' : ''}`}
          >
